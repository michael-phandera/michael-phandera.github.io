 ...
 */
function checkTxnid($txnid, $db)
{
	$results = $db->count("payments", ["txnid" =>  $txnid]);
	return !$results;
}
 ...
 */
function addPayment($data, $db)
{
	if (is_array($data)) {
		$db->insert("payments", [txnid => $data["txn_id"], payment_amount => $data["payment_amount"], payment_status => $data["payment_status"], itemid => $data["item_number"], createdtime => date("Y-m-d H:i:s")]);
		return $db->id();
	}
	return false;
}
